...
if __name__ == "__main__":
    ...
    def batch_score_func(line):
        ...
        return [score_func(src, tgt) for tgt in tgt_list]

    def batch_similarity_func(line):
        smiles_list = line.split(" ")
        src, tgt_list = smiles_list[0], smiles_list[1:]
        return [similarity(src, tgt) for tgt in tgt_list]

    def batch_diversity_func(line):
        smiles_list = line.split(" ")
        _, tgt_list = smiles_list[0], smiles_list[1:]
        return [1- similarity(smi0, smi1) for smi0, smi1 in combinations(tgt_list, 2)]
    ...
    diversities = np.nanmean(np.array(diversities), axis=1)
